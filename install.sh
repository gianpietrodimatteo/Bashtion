...
clean() {
  ...
  rm -vf ~/.bashrc ~/.bash_aliases ~/.bash_functions ~/.bash_completion ~/.bash_profile ~/.profile ~/.inputrc ~/bin/conf
}

prompt() {
  read -p "This will delete ~/.bashrc ~/.bash_aliases ~/.bash_functions ~/.bash_completion ~/.bash_profile ~/.profile ~/.inputrc ~/bin/conf. Proceed? [y]" -n 1 -r
  echo
  if [[ $REPLY =~ ^[Yy]$ ]]
  then
    clean
